...
fn main() {
    let parameters = vec![
        ...
        Complex::from_big_decimal(&BigDecimal::from(5), &BigDecimal::from(0)),  // x^2 的系数 3
        Complex::from_big_decimal(&BigDecimal::from(2), &BigDecimal::from(0)),  // x^3 的系数 2
    ];
    let equation = EquationWithDegrees::new(parameters);

    // 设置误差阈值
    let threshold = BigDecimal::from_str("0.02").unwrap();

    // 求解
    let roots = equation.solve(&threshold);

    // 打印结果
    println!("2x³+3x²+2x-1=0 的解是:");
    for root_index in 0..roots.len()-1 {
